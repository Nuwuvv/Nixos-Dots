{ config, pkgs, ... }:
let
  updateScript-sh = pkgs.pkgs.writeShellScriptBin "update" ''
...
in
{
 programs.waybar = {
   settings = {
     mainBar = {
       "custom/updates" = {
         exec = ''${updateScript-sh}/bin/update'';
         signal = 12;
         on-click = "";
         ...
         format = "{} {icon}";
         format-icons = {
           has-updates = "<span color='#FAF09D'>󰚰</span>";
           updating = "<span color='#F0B24D'></span>";
           updated = "<span color='#BADBBA'></span>";
           error = "<span color='#F37C75'></span>";
         };
       };
     };
   };
